...
@Slf4j
public class RedisCustomerAccessPermissionCache extends DefaultCustomerAccessPermissionCache implements InitializingBean {

    // 所有权限数据在redis中的key
    private static final String REDIS_ALL_PERMISSION_KEY = "user:customer:permission:all";
    ...
    // 用户权限数据在redis中的key前缀
    private static final String REDIS_USER_PERMISSION_KEY_PREFIX = "user:customer:permission:";
    ...
    // 公开权限数据在redis中的key
    private static final String REDIS_OVERT_PERMISSION_KEY = "user:customer:permission:overt";
    ...
    public List<Permission> getOwnedPermission(Long customerUserId) {
        ...
        String key = REDIS_USER_PERMISSION_KEY_PREFIX + customerUserId;
        ...
        if (CollectionUtils.isEmpty(values)) {
            ...
            try {
                ...
                if (CollectionUtils.isEmpty(values)) {
                    List<Permission> permissions = super.getOwnedPermission(customerUserId);
                    values = permissions.stream().map(p -> {
                        try {
                            return objectMapper.writeValueAsString(p);
                        } catch (JsonProcessingException e) {
