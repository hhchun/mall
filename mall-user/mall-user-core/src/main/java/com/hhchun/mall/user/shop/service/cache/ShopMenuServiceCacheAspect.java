package com.hhchun.mall.user.shop.service.cache;
...
@Component
@Aspect
public class ShopMenuServiceCacheAspect {

    private static final String REDIS_USER_MENU_KEY_PREFIX = "user:shop:user:menu:";
    ...
    @Around("execution(public  java.util.List<com.hhchun.mall.user.shop.entity.vo.ShopMenuVo> " +
            "com.hhchun.mall.user.shop.service.ShopMenuService.getShopMenusByUserId(Long,boolean,boolean))")
    public Object getShopMenusByUserIdFromCache(ProceedingJoinPoint point) throws Throwable {
        ...
        if (CollectionUtils.isEmpty(values)) {
            synchronized (key.intern()) {
                ...
                if (CollectionUtils.isEmpty(values)) {
                    ...
                    List<ShopMenuVo> mvs = (List<ShopMenuVo>) point.proceed(new Object[]{userId, upper, false});
        ...
        List<ShopMenuVo> mvs = values.stream().map(value -> {
            try {
                return objectMapper.readValue(value, ShopMenuVo.class);
            } catch (JsonProcessingException e) {
                e.printStackTrace();
                throw new UnknownErrorException(e);
            }
